...
def _restore_tailscale_backup_sync():
    """
    Corps synchrone de la restauration - exécuté dans un thread séparé
    (via asyncio.to_thread) pour ne jamais bloquer la boucle asyncio partagée
    par toutes les requêtes de l'application pendant les 4 étapes ci-dessous
    (recompilation frontend, redémarrages de services, attentes).
    """
...
async def restore_tailscale_backup(current_user: dict = Depends(get_current_user)):
    ...
    try:
        return await asyncio.to_thread(_restore_tailscale_backup_sync)
    except HTTPException:
        raise
    except Exception as e:
...
def _configure_tailscale_sync(tailscale_ip: str):
    """
    Corps synchrone de la configuration Tailscale - exécuté dans un thread séparé
    (via asyncio.to_thread). Cette séquence peut prendre plusieurs minutes
    (recompilation frontend jusqu'à 10 min, redémarrages, health-check de 30 s) ;
    l'exécuter directement dans la route async gelait toute l'application pour
    tous les utilisateurs pendant toute la durée de l'opération.
    """
...
async def configure_tailscale(
    ...
    try:
        # Toute la séquence (potentiellement plusieurs minutes) tourne dans un
        # thread à part pour ne pas geler la boucle asyncio partagée par toutes
        # les requêtes en cours pendant ce temps.
        return await asyncio.to_thread(_configure_tailscale_sync, tailscale_ip)
    except HTTPException:
        raise
