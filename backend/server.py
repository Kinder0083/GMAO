...
@api_router.get("/purchase-history/stats")
async def get_purchase_history_stats(
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
    current_user: dict = Depends(get_current_user)
):
    """Récupère les statistiques de l'historique d'achat"""
    try:
        # Filtres de date
        match_filter = {}
        if start_date:
        ...
        all_purchases = await db.purchase_history.find(match_filter).to_list(None)
        
        # Statistiques globales
        total_achats = len(all_purchases)
        montant_total = sum(p.get('montantLigneHT', 0) for p in all_purchases if p.get('montantLigneHT'))
        
        # Commandes uniques
        unique_commandes = set()
        for p in all_purchases:
            num_cmd = p.get('numeroCommande')
            if num_cmd:
                unique_commandes.add(num_cmd)
        total_commandes = len(unique_commandes)
        
        # Statistiques par utilisateur (colonne L = creationUser)
        user_stats = {}
        ...
        # Convertir monthly_stats en liste
        monthly_list = []
        ...
        return {
            ...
            'par_utilisateur': users_list,
            'par_mois': monthly_list
        }
